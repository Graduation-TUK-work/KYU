        ...
        if (pContext->ioType == IO_TYPE::READ) {
            ...
            case PKT_MOVE: {
                FPacketMove* movePkt = (FPacketMove*)pContext->buffer;
                {
                    ...
                    g_playerDataMap[id].CurrentHealth = movePkt->Data.CurrentHealth;
                    g_playerDataMap[id].bIsDowned = movePkt->Data.bIsDowned;
                    g_playerDataMap[id].bIsBeingCarried = movePkt->Data.bIsBeingCarried;
                }
            ...
            case PKT_ROLE_SELECT:
            {
...

                {
                    ...
                    RoleStatePkt.Data.CurrentHealth = Info.CurrentHealth;
                    RoleStatePkt.Data.bIsDowned = Info.bIsDowned;
                    RoleStatePkt.Data.bIsBeingCarried = Info.bIsBeingCarried;
                }

                cout << "[Role] ID " << rolePkt->PlayerId << " selected Role: " << (int)rolePkt->Role << endl;
                Broadcast((char*)&RoleStatePkt, sizeof(RoleStatePkt), pSession);
            }
            break;
            case PKT_ACTION:
            {
    ...
    while (true) {
        ...
        if (clientSocket != INVALID_SOCKET) {
            ...
            // 2. [수정사항] 데이터베이스에 즉시 등록
            {
                ...
                info.CurrentHealth = 2;
                info.bIsDowned = false;
                info.bIsBeingCarried = false;
            ...
            // 4. [수정사항] 기존 플레이어 정보 전송 (본인 제외)
            {
                ...
                for (auto it = g_playerDataMap.begin(); it != g_playerDataMap.end(); ++it)
                {
                    ...
                    oldPlayerPkt.Data.CurrentHealth = it->second.CurrentHealth;
                    oldPlayerPkt.Data.bIsDowned = it->second.bIsDowned;
                    oldPlayerPkt.Data.bIsBeingCarried = it->second.bIsBeingCarried;
